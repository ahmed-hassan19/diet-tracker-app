...
- The two-step health notice persists only `healthNoticeVersion: 1`, canonical `healthNoticeAcceptedAt`, and the existing settings timestamp. Pregnancy, trying-to-conceive, breastfeeding, eating-disorder, kidney-disease, diabetes, and clinician-diet selections remain transient UI memory and are cleared on back, refusal, acceptance, and logout. A missing or replaced acknowledgment after login, import, or remote merge reopens the gate; export, delete, and logout remain available throughout.
