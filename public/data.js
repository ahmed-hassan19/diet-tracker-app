"use strict";
/* ================= بيانات الوجبات (كميات معدّلة عن النظام الأصلي) ================= */
const MEALS = {
  b:{name:"🌅 الفطار", note:"اختار وجبة وسجّل اختلافات العبوة أو الكمية: ≈ 455–510 سعر · 29–37 جم بروتين", opts:[
    {t:"٣ بيضات + ٣ توست أسمر + خيار وطماطم", k:454, p:29, f:18, c:44},
    {t:"٢٥٠ جم جبنة قريش + ٣ توست أسمر + ملعقة صغيرة زيت زيتون + خيار", k:478, p:37, f:16, c:47},
    {t:"٧٠ جم شوفان + ٣٠٠ مل لبن قليل الدسم + نص سكوب واي", k:484, p:32, f:12, c:62},
    {t:"٧ ملاعق فول + ٣ بيضات مسلوقة + ١ توست أسمر", k:508, p:35, f:20, c:47}]},
  bc:{name:"☕ مشروب الصبح (اختياري)", note:"بيتحسب في مجموع اليوم — السويتال صفر سعرات", opts:[
    {t:"نيسكافيه Coffee Break 2×1 (ظرف ١٢ جم) + سويتال", k:55, p:1, f:3, c:6}]},
  s:{name:"🍎 سناك", note:"اختار حسب باقي يومك: ≈ 210–275 سعر · 14–20 جم بروتين", opts:[
    {t:"زبادي يوناني عالي البروتين (١٧٠ جم) + تفاحة + ١٠ جم لوز", k:266, p:20, f:6, c:33},
    {t:"٢٠٠ جم زبادي قليل الدسم + ٢٠ جم لوز", k:249, p:15, f:13, c:18},
    {t:"١٥٠ جم جبنة قريش + ١ توست أسمر + طماطم", k:210, p:20, f:6, c:19},
    {t:"كوب لبن قليل الدسم (٢٥٠ مل) + ٢٥ جم فول سوداني", k:273, p:14, f:17, c:16}]},
  l:{name:"🍗 الغدا", note:"الأوزان مطبوخة والزيت موزون: ≈ 750–810 سعر · 61–68 جم بروتين", opts:[
    {t:"٢٠٠ جم صدور فراخ مشوية + ٣٠٠ جم رز مطبوخ + سلطة + ١٠ جم زيت", k:792, p:68, f:16, c:94},
    {t:"٢٢٠ جم سمك مشوي + ٤٠٠ جم بطاطس مسلوقة + سلطة + ١٠ جم زيت", k:748, p:61, f:16, c:90},
    {t:"٢٠٠ جم لحم أحمر قليل الدهن + ٢٥٠ جم رز مطبوخ + خضار سوتيه", k:778, p:64, f:18, c:90}]},
  cf:{name:"☕ قهوة قبل التمرين (اختياري)", note:"سجّل كمية الكافيين من العبوة؛ تجنبها لو بتزود الدوخة أو الخفقان أو بتأثر على نومك، ومتتخطاش 400 مجم/اليوم", opts:[
    {t:"قهوة بن أرابيكا وسط سادة + سويتال", k:4, p:0, f:0, c:1}]},
  pw:{name:"🍌 وجبة اختيارية قبل التمرين بـ٦٠–١٢٠ دقيقة", dayNote:"لو محتاجها، كلها قبل بداية التمرين بـ٦٠–١٢٠ دقيقة؛ دي مش وجبة بعد التمرين.", note:"اختيارية قبل بداية التمرين بـ٦٠–١٢٠ دقيقة؛ لو مش محتاجها سيبها من غير اختيار: ≈ 185–255 سعر", opts:[
    {t:"سكوب واي بروتين بالمياه + موزة", k:222, p:25, f:2, c:26, legacyOnly:true},
    {t:"علبة تونة لايت مصفاة (~١٠٠ جم) + ١ توست أسمر", k:186, p:29, f:2, c:13},
    {t:"٢٠٠ جم جبنة قريش + تفاحة", k:254, p:22, f:6, c:28},
    {t:"٢٥٠ جم زبادي يوناني عالي البروتين + موزة", k:246, p:25, f:2, c:32}]},
  nt:{name:"🥤 سكوب Nitro-Tech اليومي الوحيد", dayNote:"يوم التمرين: بعد نهاية التمرين بـ٠–١٢٠ دقيقة (٦٠ دقيقة موعد عملي). يوم الراحة: مع سناك العصر. اختار واحد فقط.", note:"سكوب واحد فقط لليوم؛ بعد نهاية التمرين بـ٠–١٢٠ دقيقة، أو مع سناك العصر في يوم الراحة؛ الاختيارين شاملين ٣ جم كرياتين", opts:[
    {t:"سكوب Nitro-Tech (٤٤ جم) بالمياه + موزة", k:246, p:31, f:2.5, c:26},
    {t:"سكوب Nitro-Tech (٤٤ جم) + ٢٥٠ مل لبن قليل الدسم", k:270, p:38, f:6.5, c:15}]},
  d:{name:"🌙 العشا", note:"اختيارات كاملة بدل سناك صغير: ≈ 445–515 سعر · 35–38 جم بروتين", opts:[
    {t:"٢٥٠ جم جبنة قريش + سلطة + ملعقة صغيرة زيت زيتون + ٢ توست أسمر", k:446, p:35, f:14, c:45},
    {t:"٣ بيضات + ١٥٠ جم جبنة قريش + خضار + ١ توست أسمر", k:465, p:38, f:21, c:31},
    {t:"علبة تونة مصفاة + ٢٥٠ جم بطاطس مسلوقة + ٢ توست + سلطة بملعقة صغيرة زيت", k:512, p:37, f:8, c:73},
    {t:"٢٥٠ جم زبادي يوناني عالي البروتين + ٣٠ جم لوز + ٤٠ جم شوفان", k:488, p:36, f:20, c:41}]}
};
/* قوالب اليوم بتشير لاختيارات MEALS نفسها عشان الأرقام المعروضة والاختبارات ميتفرقوش */
const PLAN_TEMPLATES = [
  {name:"الأساسي — تمرين أو راحة", picks:{b:3,s:3,l:0,nt:0,d:3}},
  {name:"تبديل باللبن", picks:{b:0,s:3,l:0,nt:1,d:2}},
  {name:"تبديل باللحمة الحمراء", picks:{b:3,s:0,l:2,nt:0,d:3}}
];
const EXTRAS = [
  {t:"موزة", k:96, p:1, f:0, c:23},
  {t:"رغيف بلدي صغير", k:173, p:6, f:1, c:35},
  {t:"١٥٠ جم رز مطبوخ زيادة", k:183, p:3, f:1, c:41},
  {t:"فاكهة إضافية", k:80, p:0, f:0, c:20},
  {t:"٣٠ جم مكسرات", k:180, p:6, f:16, c:6},
  {t:"١٤ جم زيت في الطبيخ فوق المحسوب", k:126, p:0, f:14, c:0},
  {t:"ملعقة صغيرة عسل نحل (٧ جم)", k:24, p:0, f:0, c:6},
  {t:"ملعقة كبيرة عسل نحل (٢١ جم)", k:68, p:0, f:0, c:17},
  {t:"وحدة طاقة: موزة + ٤٠ جم شوفان + ٢٥٠ مل لبن قليل الدسم", k:370, p:15, f:7, c:62}
];
/* مرجع سعرات تقريبي؛ ملصق العبوة والوصفة الموزونة لهما الأولوية */
const CALREF=[
 {cat:"🥄 زيوت ودهون", items:[
  {t:"ملعقة كبيرة زيت (١٤ جم)", k:126, p:0, f:14, c:0},
  {t:"ملعقة صغيرة زيت (٥ جم)", k:45, p:0, f:5, c:0},
  {t:"ملعقة كبيرة زيت زيتون", k:126, p:0, f:14, c:0},
  {t:"ملعقة كبيرة سمنة بلدي", k:120, p:0, f:13, c:0},
  {t:"ملعقة كبيرة زبدة", k:100, p:0, f:11, c:0},
  {t:"ملعقة كبيرة طحينة", k:90, p:3, f:8, c:3}]},
 {cat:"🍞 عيش ونشويات", items:[
  {t:"رغيف عيش بلدي صغير", k:173, p:6, f:1, c:35},
  {t:"توست أسمر (شريحة)", k:70, p:3, f:1, c:13},
  {t:"عيش فينو (رغيف)", k:180, p:6, f:2, c:35},
  {t:"رز مطبوخ (١٠٠ جم)", k:130, p:2, f:0, c:28},
  {t:"مكرونة مطبوخة (١٠٠ جم)", k:130, p:5, f:1, c:25},
  {t:"بطاطس مسلوقة (١٠٠ جم)", k:85, p:2, f:0, c:20},
  {t:"شوفان (٥٠ جم)", k:190, p:7, f:4, c:33}]},
 {cat:"🍗 بروتين", items:[
  {t:"بيضة مسلوقة", k:78, p:6, f:5, c:1},
  {t:"صدور فراخ مشوية (١٠٠ جم)", k:165, p:31, f:4, c:0},
  {t:"وراك فراخ بالجلد (١٠٠ جم)", k:230, p:24, f:15, c:0},
  {t:"لحم أحمر قليل الدهن (١٠٠ جم)", k:190, p:28, f:8, c:0},
  {t:"كبدة (١٠٠ جم)", k:160, p:25, f:5, c:4},
  {t:"سمك مشوي (١٠٠ جم)", k:115, p:24, f:2, c:0},
  {t:"علبة تونة لايت مصفاة", k:120, p:26, f:1, c:0},
  {t:"سكوب واي بروتين", k:120, p:24, f:2, c:3},
  {t:"فول مدمس (٥ ملاعق)", k:150, p:10, f:3, c:22},
  {t:"قرص طعمية", k:90, p:3, f:6, c:7},
  {t:"سكوب Nitro-Tech (٤٤ جم) — شامل ٣ جم كرياتين", k:150, p:30, f:2.5, c:3}]},
 {cat:"🥛 ألبان", items:[
  {t:"جبنة قريش (١٠٠ جم)", k:90, p:11, f:3, c:3},
  {t:"جبنة بيضاء (٥٠ جم)", k:130, p:7, f:11, c:2},
  {t:"جبنة رومي (٣٠ جم)", k:115, p:8, f:9, c:0},
  {t:"زبادي يوناني (١٧٠ جم)", k:110, p:17, f:1, c:6},
  {t:"زبادي قليل الدسم (٢٠٠ جم)", k:125, p:11, f:3, c:14},
  {t:"كوب لبن قليل الدسم (٢٥٠ مل)", k:120, p:8, f:4, c:12}]},
 {cat:"🍌 فاكهة وسكريات", items:[
  {t:"موزة وسط", k:96, p:1, f:0, c:23},
  {t:"تفاحة وسط", k:85, p:0, f:0, c:22},
  {t:"برتقالة", k:65, p:1, f:0, c:15},
  {t:"تمرة", k:25, p:0, f:0, c:6},
  {t:"ملعقة صغيرة سكر", k:15, p:0, f:0, c:4},
  {t:"ملعقة صغيرة عسل نحل (٧ جم)", k:24, p:0, f:0, c:6},
  {t:"ملعقة كبيرة عسل نحل (٢١ جم)", k:68, p:0, f:0, c:17},
  {t:"حلاوة طحينية (٣٠ جم)", k:160, p:3, f:9, c:16},
  {t:"لوز (١٠ جم)", k:60, p:2, f:5, c:2},
  {t:"مكسرات مشكلة (٣٠ جم)", k:180, p:6, f:16, c:6}]},
 {cat:"🍲 أكلات مصرية", items:[
  {t:"طبق كشري وسط", k:550, p:14, f:12, c:95},
  {t:"حبة محشي وسط", k:50, p:1, f:2, c:7},
  {t:"طبق ملوخية بالتقلية", k:120, p:3, f:9, c:8},
  {t:"طبق فول بالزيت", k:250, p:13, f:9, c:30}]}
];
console.assert(CALREF.every(g=>g.items.every(o=>o.t&&o.k>=0&&o.p>=0)),"CALREF shape");
console.assert(
  Object.values(MEALS).flatMap(m=>m.opts).concat(EXTRAS,CALREF.flatMap(g=>g.items))
    .every(o=>o.k===0||Math.abs(o.k-(o.p*4+o.f*9+o.c*4))/o.k<=0.10),
  "food calories should reconcile with displayed macros within 10%"
);
const WORKOUTS = ["Push","Pull","Legs","كورة (حراسة)","كارديو فقط","راحة"];
/* tw = الوزن اللي الأهداف اتراجعت عنده آخر مرة؛ 0 يعني لسه متراجعتش */
const DEF={name:"",sex:"m",age:30,ht:0,act:1.375,klo:1750,khi:1850,plo:150,phi:170,sw:90,gw:80,tw:0};
