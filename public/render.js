...
function renderPlan(){
  ...
  let templateRows="";
  PLAN_TEMPLATES.forEach((template,i)=>{
    let rows="";
    Object.entries(template.picks).forEach(([key,sel])=>{
      const o=MEALS[key].opts[sel];
      rows+='<tr><td>'+MEALS[key].name+'</td><td>'+o.t+'</td><td>'+o.k+'</td><td>'+o.p+'</td></tr>';
    });
    const total=totals(template.picks);
    templateRows+='<h3>'+template.name+(i===0?' <span class="muted" style="font-weight:400">(الاختيار الأساسي)</span>':'')+'</h3>'
      +'<table><tr><th>الوجبة</th><th>الاختيار</th><th>سعر</th><th>بروتين</th></tr>'+rows
      +'<tr><th colspan="2">الإجمالي</th><th>'+total.k+'</th><th>'+total.p+' جم</th></tr></table>'
      +'<p class="muted" style="margin:6px 0 12px">دهون '+total.f+' جم · كارب '+total.c+' جم</p>';
  });
  let mealsRows="";
  for(const key in MEALS){
    const m=MEALS[key];
  ...
  <div class="card"><h2>🥤 القوالب اليومية وNitro-Tech</h2>
    ...
    <ul class="plain">
      ...
      <li><b>الكرياتين:</b> السكوب شامل ٣ جم، وهي جرعة صيانة مقبولة من غير تحميل. الجرعة اليومية الصغيرة بتملأ المخزون تدريجيًا، فمتزودهاش بسكوب تاني.</li>
      <li>سجّل تاريخ بداية Nitro-Tech في ملاحظات اليوم عشان تفسّر أي تغيير بعده في الوزن أو InBody.</li>
    </ul>
    <div class="note">💧 الكرياتين ممكن يزوّد مياه العضلات ويأثر على الميزان وقراءات InBody بدرجة بتختلف من شخص للتاني؛ ده لوحده مش سبب لتغيير السعرات.</div>
  </div>
  ...
  <div class="card"><h2>💊 المكملات — اختيارية وليست بديلًا للأكل</h2>
    <ul class="plain">
      <li><b>كرياتين مونوهيدرات:</b> لو طبيبك مأكد عدم وجود مانع كلوي أو دوائي، سكوب Nitro-Tech المخطط بيوفر ٣ جم يوميًا؛ التحميل مش ضروري. ممكن يزيد مياه العضلات والوزن بدرجة متفاوتة.</li>
      <li><b>واي بروتين:</b> مجرد طريقة مريحة لإكمال هدف البروتين. اختيار Nitro-Tech اليومي هو السكوب المخطط؛ متضاعفوش باختيار واي تاني من غير إعادة حساب اليوم، ومفيش ضرورة لتناوله فور التمرين.</li>
  ...
  <div class="card"><h2>📏 قواعد القياس الصح</h2>
    <ul class="plain">
      ...
      <li><b>خط الأساس:</b> قراءة InBody يوم ٣١ يوليو الساعة ٢٠:٠٢ (وزن ١٠٠٫٥ كجم) هي مرجع ما قبل الكرياتين فقط؛ متضيفش وزنها المسائي لسلسلة «وزن الصبح»، وخلي المتابعة اليومية بقياسات الصبح الموحّدة.</li>
      <li><b>InBody/BIA:</b> كرر القياس بعد حوالي ٤ أسابيع على نفس الجهاز وفي ظروف متقاربة للسوائل والأكل والتمرين ووقت القياس. استخدم الدهون والعضلات كاتجاه على نفس الجهاز، لأن الترطيب وخوارزمية الجهاز ممكن يغيروا التقدير.</li>
      <li><b>تفسير الاتجاه:</b> تغييرات الدهون والعضلات في مدة قصيرة ممكن تكون نسيج حقيقي أو مياه وضوضاء BIA، ومينفعش نفصل بينهم بثقة. الاتجاه الأخير حوالي ١ كجم/أسبوع قريب من الحد الأسرع المقبول، فمتقللش هدف السعرات الحالي بسببه.</li>
      <li><b>BMR في التقرير:</b> رقم محسوب بمعادلة، مش قياس مباشر للحرق، ومش سبب لاستبدال معادلة Mifflin–St Jeor أو تقليل السعرات.</li>
      ...
      <li><b>التوازن العضلي القطاعي:</b> مفيد للمتابعة، لكنه مش تصريح طبي للركبة أو بديل لتقييم الطبيب أو العلاج الطبيعي.</li>
    </ul>
  </div>
