...
**Free-text suggestions.** The add-food/add-extra box and both مرجع السعرات boxes suggest from user history (`S.foods`, `S.calref.items`) plus the built-in `MEALS`/`EXTRAS`/`CALREF` tables, via a native `<datalist>` that `addForm()`/`renderCalRef()` emit inside the same template string as the input; an exact name match autofills the four macro boxes (`fillFood`). It is not a custom dropdown because `renderDay()` replaces `#meals-box`/`#extras-box` wholesale via `innerHTML` and `mergeRemote()` can trigger that mid-typing — an imperatively attached listener would be silently lost, while inline attributes and the datalist survive the re-render.
