...
- Hardened-disabled and enabled release evidence records the log-exclusion activation as canonical UTC within 24 hours before verification and sets the conservative existing-log expiry to exactly activation plus the `_Default` 30-day retention. Preconfiguration records keep both timestamps null; past, future-activation, noncanonical, or inconsistent dates fail closed.
