...
| `state.js`  | `S`, `KEY`, `load`/`save`, `migrateReviewedProfile`, `day`, `esc`, `weightSeries`, export/import, and the state-reading accessors `T`, `getOpt`, `getExtra`, `totals`, `project`, `foodNames`, `foodByName`, `qtyNames` |
...
That last rule is **enforced, not aspirational**: `tests/unit/profile.test.mjs` `vm`-runs `calc.js` on its own, with neither `data.js` nor a state object in scope, _and_ greps the source for the forbidden names. The `vm` load alone is not the guard it looks like — a free identifier inside a function nobody calls resolves at call time, so a `calc.js` that mentions `S` still loads with exit code 0; the text assertion is what actually fails the suite. `T()`/`project()` read `S.settings` and `totals()` needs `getOpt`/`getExtra`, which is why all four sit in `state.js` despite being calculation-shaped.

**Free-text suggestions.** The add-food/add-extra box and both مرجع السعرات boxes suggest from user history (`S.foods`, `S.calref.items`) plus the built-in `MEALS`/`EXTRAS`/`CALREF` tables, via a native `<datalist>` that `addForm()`/`renderCalRef()` emit inside the same template string as the input; an exact name match autofills the four macro boxes (`fillFood`). The مرجع السعرات pair is the exception to "suggest the stored name": those titles are one string, `"النوع (الكمية)"`, so `crNames()` strips the trailing parenthetical for the النوع box and `qtyNames()` harvests it for the الكمية box — suggesting the whole title in النوع would send the quantity twice into the AI prompt and the saved name. It is not a custom dropdown because `renderDay()` replaces `#meals-box`/`#extras-box` wholesale via `innerHTML` and `mergeRemote()` can trigger that mid-typing — an imperatively attached listener would be silently lost, while inline attributes and the datalist survive the re-render.
